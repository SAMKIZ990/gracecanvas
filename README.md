...
2. Copy `.env.local.example` to `.env.local` and set `OPENAI_API_KEY` if you want AI analysis and generation.
3. Verify `DATABASE_URL="file:./dev.db"` is present in `.env.local` for the local SQLite database.
4. Run `npm run dev`.
